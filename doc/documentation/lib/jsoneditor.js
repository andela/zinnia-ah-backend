...
(function() {
  ...
  JSONEditor.prototype = {
    ...
    init: function() {
      ...
      var theme_class =
        JSONEditor.defaults.themes[
          this.options.theme || JSONEditor.defaults.theme
        ];
      if (!theme_class)
        throw "Unknown theme " +
          (this.options.theme || JSONEditor.defaults.theme);
      ...
      var icon_class =
        JSONEditor.defaults.iconlibs[
          this.options.iconlib || JSONEditor.defaults.iconlib
        ];
    ...
    expandSchema: function(schema) {
      ...
      if (schema.allOf) {
        for (i = 0; i < schema.allOf.length; i++) {
          extended = this.extendSchemas(
            extended,
            this.expandSchema(schema.allOf[i])
          );
        }
      ...
      if (schema["extends"]) {
        ...
        if (!Array.isArray(schema["extends"])) {
          extended = this.extendSchemas(
            extended,
            this.expandSchema(schema["extends"])
          );
        }
      ...
      if (schema.oneOf) {
        ...
        for (i = 0; i < schema.oneOf.length; i++) {
          extended.oneOf[i] = this.extendSchemas(
            this.expandSchema(schema.oneOf[i]),
            tmp
          );
        }
      }
      ...
      $each(obj1, function(prop, val) {
        ...
        if (typeof obj2[prop] !== "undefined") {
          ...
          // Type should be intersected and is either an array or string
            ...
            if (
              extended.type.length === 1 &&
              typeof extended.type[0] === "string"
            ) {
  ...
  JSONEditor.Validator = Class.extend({
    init: function(jsoneditor, schema) {
      ...
      this.options = {};
      this.translate =
        this.jsoneditor.translate || JSONEditor.defaults.translate;
    },
    validate: function(value) {
    ...
    _validateSchema: function(schema, value, path) {
      ...
      if (schema["extends"]) {
        for (i = 0; i < schema["extends"].length; i++) {
          errors = errors.concat(
            this._validateSchema(schema["extends"][i], value, path)
          );
        }
      }
      ...
      if (schema.allOf) {
        for (i = 0; i < schema.allOf.length; i++) {
          errors = errors.concat(
            this._validateSchema(schema.allOf[i], value, path)
          );
        }
      }
  ...
  JSONEditor.AbstractEditor = Class.extend({
    ...
    init: function(options) {
      ...
      this.formname =
        options.formname || this.path.replace(/\.([^.]+)/g, "[$1]");
    ...
    getLink: function(data) {
      ...
      var href = this.jsoneditor.compileTemplate(
        data.href,
        this.template_engine
      );
        ...
        // Range Input
          ...
          if (this.schema.multipleOf) {
            if (min % this.schema.multipleOf)
              min =
                Math.ceil(min / this.schema.multipleOf) *
                this.schema.multipleOf;
            if (max % this.schema.multipleOf)
              max =
                Math.floor(max / this.schema.multipleOf) *
                this.schema.multipleOf;
        ...
        // ACE editor for everything else
          ...
          if (JSONEditor.plugins.ace.theme)
            this.ace_editor.setTheme(
              "ace/theme/" + JSONEditor.plugins.ace.theme
            );
          // The mode
  ...
  JSONEditor.defaults.editors.object = JSONEditor.AbstractEditor.extend({
    ...
    layoutEditors: function() {
      ...
      if (this.format === "grid") {
        ...
        $each(this.property_order, function(j, key) {
          ...
          var height = editor.options.hidden
            ? 0
          ...
          if (rows[i].width < 12) {
            ...
            for (j = 0; j < rows[i].editors.length; j++) {
              if (biggest === false) biggest = j;
                ...
                rows[i].editors[j].width > rows[i].editors[biggest].width
              )
                biggest = j;
    ...
    insertPropertyControlUsingPropertyOrder: function(
      property,
      control,
      container
    ) {
    ...
    refreshAddProperties: function() {
      ...
      for (i in this.cached_editors) {
        ...
        if (
          ...
          this.addproperty_checkboxes[i].disabled = this.addproperty_checkboxes[
            i
          ].checked;
      ...
      if (this.error_holder) {
        if (my_errors.length) {
          ...
          $each(my_errors, function(i, error) {
            self.error_holder.appendChild(
              self.theme.getErrorMessage(error.message)
            );
          });
        }
        // Hide error area
  ...
  JSONEditor.defaults.editors.array = JSONEditor.AbstractEditor.extend({
    ...
    addControls: function() {
      ...
      this.delete_last_row_button.addEventListener("click", function(e) {
        ...
        if (
          self.rows.length > 1 &&
          self.rows[self.rows.length - 1].tab === self.active_tab
        )
  ...
  JSONEditor.defaults.editors.table = JSONEditor.defaults.editors.array.extend({
    ...
    build: function() {
      ...
      if (this.item_has_child_editors) {
        ...
        self.header_row.appendChild(
          self.theme.getTableHeaderCell(this.item_title)
        );
      }
    ...
    build: function() {
      ...
      for (var i = 0; i < this["enum"].length; i++) {
        this.select_options[i] =
          this.options.enum_titles[i] || "Value " + (i + 1);
  ...
  JSONEditor.defaults.editors.select = JSONEditor.AbstractEditor.extend({
    ...
    preBuild: function() {
      ...
      // Boolean
        ...
        self.enum_display = (this.schema.options &&
  ...
  JSONEditor.defaults.editors.multiselect = JSONEditor.AbstractEditor.extend({
    ...
    build: function() {
      ...
      if (
        ...
        for (i = 0; i < this.option_keys.length; i++) {
          ...
          this.select_options[this.option_keys[i]] = this.inputs[
            this.option_keys[i]
          ];
    ...
    getNumColumns: function() {
      ...
      for (var i in this.select_values) {
        ...
        longest_text = Math.max(
          longest_text,
          (this.select_values[i] + "").length + 4
        );
      }
  ...
  JSONEditor.defaults.themes.jqueryui = JSONEditor.AbstractTheme.extend({
    ...
    markTabActive: function(tab) {
      tab.className =
        tab.className.replace(/\s*ui-widget-header/g, "") + " ui-state-active";
    },
    markTabInactive: function(tab) {
      tab.className =
        tab.className.replace(/\s*ui-state-active/g, "") + " ui-widget-header";
    }
  });
  ...
  JSONEditor.defaults.translate = function(key, variables) {
    ...
    var string =
      lang[key] ||
      JSONEditor.defaults.languages[JSONEditor.defaults.default_language][key];
  ...
  for (var i in JSONEditor.defaults.editors) {
    ...
    JSONEditor.defaults.editors[i].options =
  ...
  JSONEditor.defaults.resolvers.unshift(function(schema) {
    ...
    if (
      schema.type === "string" &&
      schema.media &&
      schema.media.binaryEncoding === "base64"
    ) {
      return "base64";
    }
  });
